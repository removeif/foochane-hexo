...
summary: 关键词： ubuntu16.04、Hadoop、分布式集群、环境搭建
...
$ sudo tar -zxvf hadoop-2.7.1.tar.gz -C /usr/local/bigdata/hadoop
```

## 6 修改hadoop配置文件
需要修改6个文件，文件位于`/usr/local/bigdata/hadoop/hadoop-2.7.1/etc/hadoop/`下
...
<configuration>
        ...
        <property>
                ...
                <value>file:/usr/local/bigdata/hadoop/hadoop-2.7.1/tmp</value>
...
<configuration>
        ...
        <property>
                ...
                <value>file:/usr/local/bigdata/hadoop/hadoop-2.7.1/tmp/dfs/name</value>
        </property>
        <property>
                <name>dfs.datanode.data.dir</name>
                <value>file:/usr/local/bigdata/hadoop/hadoop-2.7.1/tmp/dfs/data</value>
        </property>
</configuration>
```
...
```
export PATH=$PATH:/usr/local/bigdata/hadoop/hadoop-2.7.1/bin:/usr/local/bigdata/hadoop/hadoop-2.7.1/sbin
```
## 8 slave节点配置
...
在master节点：
```bash
$ sudo rm -r /usr/local/bigdata/hadoop/hadoop-2.7.1/tmp     # 删除 Hadoop 临时文件，如果之前有启动过
$ sudo rm -r /usr/local/bigdata/hadoop/hadoop-2.7.1/logs/*   # 删除日志文件
...
## 9 启动Hadoop集群
**在Master上执行**
首次运行需要，执行`$ hdfs namenode -format `格式化名称节点，然后就可以启动hadoop了。
